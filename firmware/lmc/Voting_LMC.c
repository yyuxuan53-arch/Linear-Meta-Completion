...
/* Final physical firmware (29 August): voting task + LMC. */

#define OPINION_B                  0U
#define OPINION_A                  1U
#define SAME_REQUIRED              10U
...
volatile uint8_t received_opinion = OPINION_B;
volatile uint16_t received_belief = 0;
volatile uint8_t received_new_message = 0;
volatile uint8_t tx_busy = 0;

uint8_t opinion = OPINION_B;
...
static void prepare_message(void)
{
    ...
    tx_message.data[0] = opinion;
    tx_message.data[1] = (uint8_t)(encoded & 0xFFU);
    tx_message.data[2] = (uint8_t)((encoded >> 8) & 0xFFU);
...
void message_rx(message_t *message, distance_measurement_t *distance)
{
    ...
    if (!startup_finished || meta_complete ||
        message_crc(message) != message->crc || message->type != NORMAL ||
        message->data[0] > OPINION_A)
    {
        return;
    }

    encoded = (uint16_t)message->data[1] |
              ((uint16_t)message->data[2] << 8);
    if (encoded > BELIEF_SCALE) encoded = BELIEF_SCALE;
    received_opinion = message->data[0];
    received_belief = encoded;
    received_new_message = 1;
}
...
static void update_colour(void)
{
    ...
    else if (opinion == OPINION_A) set_color(RGB(0,3,0));
...
static void process_received_message(void)
{
    ...
    if (!received_new_message || meta_complete) return;
    received_new_message = 0;
    neighbour = (float)received_belief / (float)BELIEF_SCALE;

    if (received_opinion != opinion)
    {
        opinion = received_opinion;
        same_count = 0;
        local_complete = 0;
        meta_belief = 0.0f;
        return;
    ...
    if (same_count < SAME_REQUIRED) same_count++;
    if (same_count >= SAME_REQUIRED)
    {
        same_count = SAME_REQUIRED;
        local_complete = 1;
    }
